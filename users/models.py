...
#         user = self.model(is_superuser=False, is_staff=False, **extra_fields)
#         user.set_unusable_password()
#         user.save(using=self._db)
#         return user

#     def create_superuser(
#         self, user_id: str, password: str | None, **extra_fields: tuple
#     ) -> User:
#         if not user_id:
#             raise ValueError("Please provide an user_id")
#         user = self.model(
#             user_id=user_id, is_superuser=True, is_staff=True, **extra_fields
#         )
...
#         self, defaults: dict | None, **kwargs: dict
#     ) -> tuple[User, bool]:
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    **Parameters:**
    ...
    height = models.OneToOneField(
        HeightModel, on_delete=models.CASCADE, related_name="Рост", verbose_name="Рост"
    )
    weight = models.OneToOneField(
        WeightModel, on_delete=models.CASCADE, related_name="Вес", verbose_name="Вес"
    )
