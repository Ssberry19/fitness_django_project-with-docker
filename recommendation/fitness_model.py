...
try:
    ...
    print("TensorFlow import failed. Using mock model as fallback.")
    
# Check for Apple Silicon and provide guidance if needed
...
try:
    ...
    if platform.system() == "Darwin" and platform.machine() == "arm64":
        ...
        if not TF_AVAILABLE:
            print("Apple Silicon (M1/M2/M3) detected. For TensorFlow support:")
            print("1. Install Miniforge from https://github.com/conda-forge/miniforge")
            print("2. Create a conda environment: conda create -n fitness python=3.9")
            print("3. Install TensorFlow: conda install -c apple tensorflow-deps")
            print("4. Then: pip install tensorflow-macos tensorflow-metal")
except:
    pass

class FitnessRecommendationModel:
    """
    Fitness recommendation model for inference
    Loads and uses the trained deep learning model
    """
    def __init__(self):
        self.model = None
        self.encoders = None
        ...
        # Only attempt to load model if TensorFlow is available
        if self.tf_available:
            self.load_model()
        else:
            print("Using mock model implementation due to TensorFlow unavailability")
    
    def load_model(self):
        """Load the trained model and required preprocessing components"""
        ...
        tokenizers_path = os.path.join(model_dir, 'tokenizers.pkl')
        if os.path.exists(tokenizers_path):
            try:
                import pickle
                with open(tokenizers_path, 'rb') as f:
                    self.tokenizers = pickle.load(f)
                print(f"Loaded tokenizers from {tokenizers_path}")
            except Exception as e:
    ...
    def preprocess_input(self, user_data):
        """
        Preprocess user input data to model-compatible format
        
        Parameters:
        user_data (dict): Dictionary with user information
            Required keys: gender, age, height, weight, goal, activity_level
            Optional keys: target_weight, cycle_start_date
        
        Returns:
        numpy.ndarray: Preprocessed features
        """
        # If TensorFlow not available or model/encoders not loaded, return None
        if not self.tf_available or self.model is None or self.encoders is None:
            return None
        ...
        # Calculate target BMI if not provided
        ...
        # Process cycle data for women
        gender = user_data['gender']
        if gender == 'F' and 'cycle_start_date' in user_data and user_data['cycle_start_date']:
            try:
                if isinstance(user_data['cycle_start_date'], str):
                    cycle_date = datetime.strptime(user_data['cycle_start_date'], "%Y-%m-%d")
                else:
                    cycle_date = user_data['cycle_start_date']
                
                days_since = (datetime.now() - cycle_date).days
                # Normalize to [0, 1] range (assuming 28-day cycle)
                normalized_days = min(days_since / 28.0, 1.0)
            ...
            # For men or women without cycle data
            normalized_days = -1
        ...
        features = {
            ...
            'normalized_days_since_cycle': normalized_days,
            'cycle_phase': cycle_phase
        }
        
        # Encode features using loaded encoders
        try:
            # Handle categorical features
            categorical_features = ["gender", "goal", "activity_level", "bmi_category"]
            encoded_categoricals = []
            
            for feature in categorical_features:
                encoder = self.encoders.get(feature + "_encoder")
                if encoder and feature in features:
                    # Reshape for encoder
                    value = features[feature]
                    if isinstance(value, list):
                        value = value[0]  # Take first value if list
                    value_reshaped = np.array([value]).reshape(-1, 1)
                    encoded = encoder.transform(value_reshaped)
                    encoded_categoricals.append(encoded)
            
            # Handle numerical features
            numerical_features = ["age", "height", "weight", "bmi", "target_weight", "target_bmi"]
            numerical_values = np.array([[features[f] for f in numerical_features]])
            
            scaler = self.encoders.get("numerical_scaler")
            if scaler:
                scaled_numericals = scaler.transform(numerical_values)
            else:
                # If no scaler, use simple normalization
                scaled_numericals = numerical_values / np.array([[100, 200, 100, 40, 100, 40]])
            
            # Handle cycle data
            cycle_data = np.array([[features['normalized_days_since_cycle']]])
            
            # Handle cycle phase
            cycle_phase_encoder = self.encoders.get("cycle_phase_encoder")
            if cycle_phase_encoder:
                cycle_phase_reshaped = np.array([features['cycle_phase']]).reshape(-1, 1)
                encoded_cycle_phases = cycle_phase_encoder.transform(cycle_phase_reshaped)
            else:
                # Default encoding if encoder not available
                encoded_cycle_phases = np.zeros((1, 5))
                phase = features['cycle_phase']
                if phase == "menstrual":
                    encoded_cycle_phases[0, 0] = 1
                elif phase == "follicular":
                    encoded_cycle_phases[0, 1] = 1
                elif phase == "ovulatory":
                    encoded_cycle_phases[0, 2] = 1
                elif phase == "luteal":
                    encoded_cycle_phases[0, 3] = 1
                else:  # not_applicable or unknown
                    encoded_cycle_phases[0, 4] = 1
            
            # Calculate weight change percentage
            weight_change_pct = ((features['target_weight'] - features['weight']) / features['weight']) * 100
            # Use global stats for standardization (approximated from training data)
            weight_change_mean = 0
            weight_change_std = 10
            weight_change_scaled = np.array([[(weight_change_pct - weight_change_mean) / weight_change_std]])
            ...
            preprocessed_features = np.concatenate(
                ...
                encoded_categoricals + 
                [cycle_data] + 
                [encoded_cycle_phases] + 
                [weight_change_scaled],
                axis=1
            )
    ...
    def generate_recommendation(self, user_data):
        """
        Generate personalized fitness recommendation
        
        Parameters:
        user_data (dict): Dictionary with user information
        
        Returns:
        dict: Recommendation components and full recommendation text
        """
        # Check if TensorFlow is available and model is loaded
        if not self.tf_available or self.model is None:
            # Fall back to mock implementation
            mock_model = MockFitnessRecommendationModel()
            return mock_model.generate_recommendation(user_data)
        
        # Preprocess input
        preprocessed_input = self.preprocess_input(user_data)
        ...
        try:
            ...
            # Process predictions to generate recommendation components
            # This would depend on the specific model architecture and outputs
...
class MockFitnessRecommendationModel:
    """
    Mock implementation of the fitness recommendation model
    Used as fallback when the real model is not available
    """
    def generate_recommendation(self, user_data):
